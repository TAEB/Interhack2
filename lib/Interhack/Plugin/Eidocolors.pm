...
# enchantment (originally from doy) {{{
    if (!$ece{enchantment})
    {
        $self->recolor(qr/\s\+0/               => $ece{plus0} || "brown");
        $self->recolor(qr/\s\+[1-3]/           => $ece{plus13} || $ece{plus} || "green");
        $self->recolor(qr/\s\+[4-9]\d*/        => $ece{plus4} || $ece{plus} || "bgreen");
        $self->recolor(qr/(?<!AC)\s\-[1-9]\d*/ => $ece{minus} || "red");
    }
# }}}
